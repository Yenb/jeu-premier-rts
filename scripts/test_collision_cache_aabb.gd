...
# recalcul (par exemple cache pris obsolete au tick precedent), ce test rougit.
#
# Protocole : deux entites-boites recouvrantes, un tick de reference (cache
# rafraichi par tick lui-meme), un tick apres purge explicite du cache (fallback
# _aabb_entite). Les deux ticks doivent rendre le meme nombre de contacts, la
# meme profondeur et la meme normale.
...
func _executer() -> void:
	# Deux ticks separes (monde + entites neufs a chaque fois), delta identique.
	# Tick 1 : le cache est ecrit par la pre-passe de tick. Tick 2 : le cache est
	# explicitement absent, tick doit le calculer via _aabb_entite. Les contacts
	# rendus doivent etre identiques bit pour bit sur nombre, normale et profondeur.
	var c1: Array = _un_tick(true)
	var c2: Array = _un_tick(false)
...
func _un_tick(avec_cache_seed: bool) -> Array:
	var monde = Monde.new()
	...
	return Collision.tick(monde, [a, b], 0.016)
