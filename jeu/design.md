...
## Transfert DISCRET conditionné par pente critique (sandpile)

ÉCART AVEC LE DESIGN INITIAL — mécanisme neuf ajouté au su de Yael
(chantier « transfert discret conditionné par pente critique »,
`scripts/sandpile.gd`, `CARTE.md` §2). NEUVIÈME nature de mécanisme dans
la taxonomie ouverte par `ecoulement.gd` : proche du « transfert conservé
pair-à-pair par comparaison relative » mais DISTINCTE sur deux axes non
composables dans un même fichier — GRANULARITÉ (1 grain entier par
déclenchement, jamais une fraction) et SEUIL (rien ne bouge sous le seuil,
là où `ecoulement.gd` transfère au moindre écart). Standard académique
Bak-Tang-Wiesenfeld (1987), automates cellulaires granulaires — angle de
repos naturel, cône au repos sans code d'agencement. Chaque paire non
ordonnée est évaluée UNE fois par tick : ne jamais reparcourir `(j,i)`
après `(i,j)` — la nature discrète du transfert n'amortit pas
l'oscillation que la nature continue de `ecoulement.gd` amortissait.
La quantité est TOUJOURS 1, garantie du mécanisme, jamais déléguée à
l'appelant. NOTE FRAMEWORK : présent dans le dépôt orion, ABSENT de la
copie locale `scripts/` de ce dépôt.
...
## Mouvement kinématique en donnée pure, partagé par tout mobile

ÉCART AVEC LE DESIGN INITIAL — mécanisme ajouté à la copie locale de
`scripts/` (`scripts/mouvement_kinematic.gd`, chantier « mouvement
kinematic partagé », `SUIVI.md` commits `8504a51 / 8c2b9c5 / 7d90e8e`,
`CARTE.md` §2). Prolongement direct de « Verticalité » : la doctrine
« Z hors position, altitude en propriété nommée » se donne UNE
implémentation partagée du pas vertical — gravité, snap au sol via
`carte_terrain.sommet`, blocage terrain/pente, plafond voxel, collision
inter-entités — au lieu de trois réécritures divergentes (joueur, cubes,
producteurs, chacune portait un bug différent). Le module est AGNOSTIQUE
DU TYPE : ne lit que des propriétés (`velocite`, `gravite`, `formes`,
`rayon_capsule`...), ne connaît ni « joueur » ni « cube » ni « mur ».
Le profil (`complet`/`simple`/`minimal`) choisit la RICHESSE de la
simulation, jamais l'identité du mobile — un cube en `complet` et un
joueur en `complet` suivent exactement le même chemin. L'appelant POSE
l'intention (velocité horizontale voulue, saut demandé) dans l'entité
AVANT l'appel ; le module ne lit aucun `Input`. Aucune physique Godot
en autorité : la doctrine « le monde vit en donnée, le physique est un
rendu » s'applique au vertical comme au reste. NOTE FRAMEWORK : présent
dans cette copie locale, ABSENT du dépôt orion — divergence assumée
tant qu'un pas vertical partagé n'est pas remonté au framework.
...
### Mesh catalogue : le rendu déclaré en donnée

ÉCART AVEC LE DESIGN INITIAL — analogue rendu de « Tout est objet », ajouté
à la copie locale (`scripts/mesh_catalogue.gd` + `data/mesh.json`,
`CARTE.md` §2). « Tout est objet » interdit qu'une catégorie du monde
s'écrive en dur dans le moteur ; le catalogue de mesh y transpose la
même règle côté rendu : les FAMILLES GÉOMÉTRIQUES (`boite` aujourd'hui,
autres à venir) sont un `case` dans `fabriquer_mesh`, les MESHES CONCRETS
vivent dans `data/mesh.json` (`taille`, `couleur`, ...), les
CONSOMMATEURS (`peuplement.gd` et autres) ne parlent que d'une clé
opaque. Ajouter un mesh nouveau = une entrée de plus dans le JSON, zéro
ligne de code chez les consommateurs. Ajouter une famille géométrique
nouvelle = un `case` de plus dans le catalogue, zéro ligne ailleurs.
Aucun cache interne : le catalogue est petit, un `charger()` coûte une
lecture JSON ; un cache aurait à se justifier par une mesure, jamais par
un réflexe. NOTE FRAMEWORK : présent dans cette copie locale, ABSENT du
dépôt orion.
...
## Collision généraliste en donnée pure (proto)

ÉCART AVEC LE DESIGN INITIAL — proto ajouté à la copie locale
(`jeu/Proto/collision.gd`, spec `jeu/Proto/COLLISION.md`, `SUIVI.md`
commit `2c39564` 2026-09-02, `CARTE.md` §2). Prolongement direct du
paragraphe ci-dessus (terrain en cases-objets, altitude en propriété
nommée) : la doctrine « le monde vit en donnée, la physique Godot n'est
qu'un rendu » se donne UNE implémentation partagée de la collision
générale, entièrement en donnée. Tout statique, aucun état interne :
chaque fonction reçoit ce dont elle a besoin. AUCUNE physique Godot en
autorité — pas de `PhysicsServer3D`, pas de `StaticBody3D`, pas de
`CollisionShape3D`. Une forme est un `Dictionary { type, transform_locale,
parametres }` (quatre types : `sphere`, `boite`, `capsule` axe Y,
...
mesh catalogue. Statut PROTO : vit dans `jeu/Proto/`, pas dans
`scripts/` ; candidat à devenir un mécanisme framework partagé.
...
### Rendu MultiMesh partagé pour une population homogène (peuplement)

ÉCART AVEC LE DESIGN INITIAL — extension du régime « accroché/décroché »
ci-dessus (`scripts/peuplement.gd`, `SUIVI.md` commit `c571a3d` +
`CANEVAS DE POPULATIONS MASSIVES` du CLAUDE.md, `CARTE.md` §2). Le régime
initial suppose UN rendu par colon (mesh accroché dessus quand la
distance est courte, décroché sinon) ; une population homogène de
milliers d'individus le paye cher — un `Node3D` par individu, plusieurs
Timers, un dispatch par tick. Le canevas est différent : UN Node qui
tient un `Array` d'individus itéré en UNE boucle `_physics_process`, UN
MultiMesh + UNE instance `RenderingServer` qui rend TOUS les individus en
UN draw call, AUCUN Node ni Timer par individu. Règle doctrinale : UN
peuplement = UN type = UN MultiMesh — un banc qui veut plusieurs meshes
côte à côte instancie plusieurs peuplements. C'est ce qui évite qu'un
peuplement devienne un aiguilleur et retombe dans « les collectifs
n'existent pas » (voir plus bas). Le module ne connaît AUCUN nom de
contenu : consomme un `type_id` opaque (résolu via `objet.gd:fabriquer`)
et un `mesh_ref` opaque (résolu via `mesh_catalogue.gd`), le CÂBLAGE du
banc appelant est la seule couche qui choisit. Pool de slots fixe alloué
à la première fabrication, LIFO, slots inactifs à échelle NULLE (jamais
`Transform3D()` identité — un mesh visible à l'origine, piège documenté).
NOTE FRAMEWORK : présent dans cette copie locale, ABSENT du dépôt orion.
...
## Framework de tick modulable : la politique appartient au JEU

ÉCART AVEC LE DESIGN INITIAL — ce dépôt affirmait « aucun accumulateur
global, pas de boucle de tick centrale » ; le module `scripts/tick.gd`
(chantier `SUIVI.md` `59c2fb0` + `8504a51`, `CARTE.md` §2) précise et
tient l'écart au su de Yael. Le module ne devient PAS une boucle
centrale : il DÉCIDE quand appeler `mouvement_kinematic.gd:pas` sur
UNE entité et avec QUEL profil, puis délègue. La décision est un
`Callable` déjà lié par l'appelant, jamais recalculé ici — patron
identique à `scripts/boucle.gd` (« la boucle ne fait que déclencher,
jamais calculer », CLAUDE.md § Règle d'état). Trois politiques offertes,
chacune faite pour un TYPE DE JEU différent :

- `politique_intrinseque` pour un MONDE ÉMERGENT (écosystème, saisons,
  faim, cycle jour/nuit qui affecte tout) — le temps du monde doit être
  UNIFORME, un hiver qui commence à T commence à T partout ; ralentir
  une zone lointaine créerait des décalages temporels visibles quand le
  joueur y arrive. Dwarf Fortress, RimWorld, roguelike survival.
- `politique_distance` pour un JEU ACTION CENTRÉ sur un point chaud
  (siège, tower defense, arène, boss avec vagues) — le gameplay se joue
  autour d'un lieu, un léger décalage des entités lointaines est
  invisible et acceptable. They Are Billions, Kingdom Rush. NE PAS
  utiliser pour un monde émergent.
- `politique_groupe` pour tests, debug, cinématiques scriptées.

Le choix appartient au JEU, jamais au module — c'est exactement la
raison pour laquelle un tick centralisé n'était PAS dans le design
initial. La factorisation vaut d'être écrite ici parce que le jeu bâti
sur ce framework a maintenant un `mouvement_kinematic.gd` partagé, et
qu'une seule décision de cadence par entité était réécrite dans chaque
manager. NOTE FRAMEWORK : présent dans cette copie locale, ABSENT du
dépôt orion.
