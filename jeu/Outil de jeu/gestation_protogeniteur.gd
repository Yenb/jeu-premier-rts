...
const CHEMIN_SCENE := "res://jeu/Outil de jeu/protogeniteur.tscn"

# PARAMETRES DE GESTATION -- exportes pour que Yael les regle dans
# l'inspecteur, directement sur le nœud enfant du geniteur.
@export var seuil_ponte: float = 120.0
@export var duree_gestation: float = 240.0
@export var cout_ponte: float = 120.0
@export var max_vivants: int = 3
...
@export var rayon_pose_min: float = 8.0
@export var rayon_pose_max: float = 12.0
@export var essais_max: int = 8

const REF_REPRODUCTION := "protogeniteur"
const NOM_GROUPE_VIVANTS := "protogeniteur"
...
static var _prochaine_graine := 20280322
...
func _ready() -> void:
	...
	if not _geniteur.has_method("stock_courant") or not _geniteur.has_method("retirer_stock"):
		push_error("gestation_generateur_public.gd : le parent n'expose pas stock_courant()/retirer_stock() -- pas le bon type de parent ?")
		return
