...
@export var seuil_ponte: float = 10.0
@export var duree_gestation: float = 60.0
@export var cout_ponte: float = 15.0
@export var max_vivants: int = 8
...
func _ready() -> void:
	...
	if not _geniteur.has_method("stock_public_courant") or not _geniteur.has_method("preleve_stock_public"):
		push_error("gestation_generateur_public.gd : le parent n'expose pas stock_public_courant()/preleve_stock_public() -- pas le bon type de parent ?")
		return
...
func _tick() -> void:
	...
	var stock: float = float(_geniteur.stock_public_courant())
	...
	if gestation.get("naissance_prete", false):
		...
		if _pondre_un_generateur():
			...
			_geniteur.preleve_stock_public(cout_ponte)
